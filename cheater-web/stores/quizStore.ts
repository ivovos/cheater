...
interface QuizState {
  ...
  generateQuiz: (imageUri: string) => Promise<{ quiz: Quiz; title: string; subject?: string }>;
  startQuiz: () => void;
...
export const useQuizStore = create<QuizState>((set, get) => ({
  ...
  // Generate new quiz
  generateQuiz: async (imageUri: string) => {
    set({ isLoading: true, error: null });

    try {
      // Generate quiz using AI service (returns quiz + title + subject)
      const result = await aiService.generateQuiz(imageUri);

      set({ currentQuiz: result.quiz, isLoading: false });
      return result;
    } catch (error: any) {
